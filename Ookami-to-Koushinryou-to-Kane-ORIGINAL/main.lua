function love.load()
   ...
   -- OTHERS --
   states = {false, false, false, false, false, false, false, false, false, true}
   last_state = 1
   i = 2
   dtotal = 0
   item1 = {15, 15}
...
function love.update(dt)
   ...
   if (checkState() < 8) then
	 ...
	 item1[i] = math.abs(math.random(item1[i - 1] - 5, item1[i - 1] + 5))
	 adjustPrice(item1[i], item1)
	 item1.x[i] = item1.x[i - 1] + 1
	 
	 item2[i] = math.abs(math.random(item2[i - 1] - 10, item2[i - 1] + 10))
	 adjustPrice(item2[i], item2)
	 item2.x[i] = item2.x[i - 1] + 1
	 
	 item3[i] = math.abs(math.random(item3[i - 1] - 15, item3[i - 1] + 15))
	 adjustPrice(item3[i], item3)
	 item3.x[i] = item3.x[i - 1] + 1
	 
	 item4[i] = math.abs(math.random(item4[i - 1] - 20, item4[i - 1] + 20))
	 adjustPrice(item4[i], item4)
	 item4.x[i] = item4.x[i - 1] + 1
	 
	 item5[i] = math.abs(math.random(item5[i - 1] - 30, item5[i - 1] + 30))
...
function love.mousepressed(x, y, button)
   if (button == "l") then
	 ...
	 if ((x >= 400 and y >= 350) and (x <= 840 and y <= 380)) then setStatesFalse(1) end
